...
type ContentDetailType = "text" | "textarea" | "image" | "video" | "file";
...
const contentImageInfo = reactive<ImageInfo>({
  fileSize: "",
  width: 0,
  height: 0
});
...
const categoryOptions = computed(() => getCategoryRows(moduleCode.value));
...
): detail is ContentDetail & { type: "text" | "textarea" } =>
...
const form = reactive({
  ...
  description: "",
  displayGroups: [] as string[],
  scheduledPublish: false,
  status: "enabled" as ModuleContentStatus
});
const editContent = computed(() =>
  editId.value ? getContentById(moduleCode.value, editId.value) : undefined
);
...
watch(
  ...
  content => {
    ...
    form.imageTitle = content?.imageTitle ?? "";
    form.imageAlt = content?.imageAlt ?? "";
    form.contentDetails =
      content?.contentDetails.map(detail => ({
        ...detail,
        links: detail.links?.map(link => ({ ...link })) ?? []
      })) ?? [];
    contentImageInfo.fileSize = content?.imageFileSize ?? "";
    contentImageInfo.width = content?.imageWidth ?? 0;
    contentImageInfo.height = content?.imageHeight ?? 0;

    if (form.image && (!contentImageInfo.width || !contentImageInfo.height)) {
      loadImageDimensions(form.image, contentImageInfo);
    }
    ...
    form.displayGroups = content?.displayGroups
      ? [...content.displayGroups]
      : [];
    form.scheduledPublish = content?.scheduledPublish ?? false;
...
function resetImageInfo(target: ImageInfo | ContentDetail) {
  target.fileSize = "";
  target.width = 0;
  target.height = 0;
}

function loadImageDimensions(url: string, target: ImageInfo | ContentDetail) {
...
function handleImageUpload(file: any) {
  ...
  form.image = URL.createObjectURL(rawFile);
  contentImageInfo.fileSize = formatFileSize(rawFile.size ?? 0);
  contentImageInfo.width = 0;
  contentImageInfo.height = 0;
  loadImageDimensions(form.image, contentImageInfo);
  return false;
}
...
const handlePreviewImage = (event?: MouseEvent) => {
  handlePreviewUploadedImage(form.image, event);
};
...
const handleRemoveImage = () => {
  ...
  resetImageInfo(contentImageInfo);
};

const handleRemoveContentDetailUpload = (detail: ContentDetail) => {
...
function handleContentDetailUpload(detail: ContentDetail, file: any) {
  const rawFile = file.raw ?? file;
  if (detail.value.startsWith("blob:")) URL.revokeObjectURL(detail.value);
...
function handleImageTitleInput(value: string) {
  form.imageTitle = value;
  form.imageAlt = value;
}

function handleImageAltInput(value: string) {
  form.imageAlt = value;
  form.imageTitle = value;
}

function handleDetailImageTitleInput(detail: ContentDetail, value: string) {
  detail.title = value;
  detail.alt = value;
}

function handleDetailImageAltInput(detail: ContentDetail, value: string) {
  detail.alt = value;
  detail.title = value;
}

function handleAddContentDetail(type: ContentDetailType) {
...
function handleAddDetailLink(detail: ContentDetail) {
  if (!isLinkableDetail(detail)) return;
  detail.links = detail.links ?? [];
  detail.links.push(createTitleLink());
}

function handleRemoveDetailLink(detail: ContentDetail, index: number) {
  if (!isLinkableDetail(detail) || !detail.links) return;
  detail.links.splice(index, 1);
}

function getDetailPreviewSegments(detail: ContentDetail) {
...
function resetForm() {
  ...
  form.contentDetails = [];
  form.title = "";
  form.keywords = getDefaultKeywords();
  form.description = "";
  form.displayGroups = [];
  form.scheduledPublish = false;
  form.status = "enabled";
...
async function handleConfirm() {
  ...
  try {
    saveContent({
      ...
      imageFileSize: contentImageInfo.fileSize,
      imageWidth: contentImageInfo.width,
      imageHeight: contentImageInfo.height,
      title: form.title,
      keywords: form.keywords,
      description: form.description,
      displayGroups: form.displayGroups,
      contentDetails: form.contentDetails,
      scheduledPublish: form.scheduledPublish,
      status: form.status
    });
  ...
  <el-card>
    ...
    >
      ...
      <el-form-item label="内容图片">
        <div class="content-image-upload-field">
          <el-upload
            ...
            accept="image/*"
          >
            <div v-if="form.image" class="content-image-preview">
              <img
                :src="form.image"
                :alt="form.imageAlt"
            ...
            <span v-else class="content-image-upload-plus" aria-hidden="true" />
          </el-upload>
          <div v-if="form.image" class="content-image-upload-meta">
            <span>图片大小：{{ contentImageInfo.fileSize || "未知" }}</span>
            <span v-if="contentImageInfo.width && contentImageInfo.height">
              图片尺寸：{{ contentImageInfo.width }} ×
              {{ contentImageInfo.height }}px
            </span>
            <span v-else>图片尺寸：读取中</span>
          </div>
          <div v-if="form.image" class="image-seo-fields">
            <el-input
      ...
      <el-form-item label="定时发布">
        <el-switch v-model="form.scheduledPublish" />
      </el-form-item>
      <el-form-item label="展示分组">
        <el-select
